...
For each study and set of comparisons, we need to calculate the compositional similarity. We're going to set this up to run in parallel. I'm using the `purrr::map` functions here (made to run in parallel using the `furrr` package), but there are other options, like `for` loops or `apply`.
